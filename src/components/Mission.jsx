    ...
    <section id="mission" className="section-shell py-4 sm:py-6" aria-labelledby="mission-heading">
      <div className="section-block px-6 py-14 sm:px-10 sm:py-20 lg:px-14">
        <div className="mx-auto max-w-3xl text-center">
          <p className="eyebrow mb-4 text-bronze-light">Our Mission</p>
          <h2 id="mission-heading" className="display-md text-white">
            Our Mission
          </h2>
        ...
        <div className="mt-12 grid gap-4 sm:mt-16 sm:grid-cols-3 sm:gap-5">
          {pillars.map((pillar) => (
            <article
              key={pillar.title}
              className={`group rounded-2xl p-7 transition-transform duration-300 hover:-translate-y-1 ${pillar.cardClass}`}
            >
              <div className={`mb-5 inline-flex rounded-full bg-void/50 p-3 ${pillar.accent}`}>
                {pillar.icon}
              </div>
              <h3 className="text-lg font-semibold tracking-tight text-white">{pillar.title}</h3>
              <p className="mt-2 text-[15px] leading-relaxed text-muted">{pillar.description}</p>
            </article>
          ))}
        </div>
      </div>
